...
if [ -f "$SOLR_VAR_DIR/data/schema.xml" ]; then
  echo -e "\n$SOLR_VAR_DIR/data/schema.xml already exists. Skipping install ...\n"
else
  cp "$SOLR_INSTALL_DIR/server/solr/schema.xml" "$SOLR_VAR_DIR/data/schema.xml"
fi
